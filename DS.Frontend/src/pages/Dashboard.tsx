...
const Dashboard = () => {
  ...
  const generateMockFiles = () => {
    const baseFiles = [
      { id: 1, name: "Project_Proposal.pdf", type: "pdf", size: "2.4 MB", uploadDate: "Today, 2:30 PM", icon: <FileIcon className="text-neon-blue" /> },
      { id: 2, name: "Team_Photo.jpg", type: "image", size: "3.8 MB", uploadDate: "Today, 10:15 AM", icon: <ImageIcon className="text-neon-purple" /> },
      { id: 3, name: "Meeting_Notes.docx", type: "document", size: "1.2 MB", uploadDate: "Yesterday", icon: <FileTextIcon className="text-neon-cyan" /> },
      { id: 4, name: "Product_Demo.mp4", type: "video", size: "18.7 MB", uploadDate: "2 days ago", icon: <Video className="text-rose-500" /> },
      { id: 5, name: "Financial_Report.xlsx", type: "document", size: "3.1 MB", uploadDate: "3 days ago", icon: <FileTextIcon className="text-green-500" /> }
    ];
    ...
    for (let i = 0; i < 4; i++) {
      baseFiles.forEach((file, index) => {
        extendedFiles.push({
          ...
          name: `${file.name.split('.')[0]}_v${i+2}.${file.name.split('.')[1]}`,
