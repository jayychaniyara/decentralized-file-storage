...
const CounterAnimation = ({ end, duration = 2000, label, icon }: { end: number, duration?: number, label: string, icon: React.ReactNode }) => {
  ...
  return (
    <div className="glass-card p-6 rounded-xl text-center animate-counter-up">
      <div className="flex items-center justify-center mb-4">
        <div className="p-3 rounded-lg glass-card inline-block">
          {icon}
        </div>
      </div>
      <div className="text-4xl font-bold mb-2 text-gradient">{count.toLocaleString()}</div>
